...
// macchina con l'indirizzo del server e il nome del pc — `defender@pc`.
//
...
pc === "attacker@attacker" ? ok(`il terminale di sinistra e' ${pc}`) : ko(`il terminale di sinistra dice ${pc}, atteso attacker@attacker`);
srv === "defender@defender" ? ok(`il terminale di destra e' ${srv}`) : ko(`il terminale di destra dice ${srv}, atteso defender@defender`);
...
visto[0] = "";
await digita(0, "ssh -o StrictHostKeyChecking=accept-new defender@10.10.0.2\n");
await pausa(11000);
await digita(0, "lab\n");                    // la password di defender
await pausa(11000);
visto[0] = "";
...
dentro === "defender@defender"
    ? ok(`dentro la sessione ssh sei ${dentro}`)
    : ko(`dentro la sessione ssh il prompt dice ${dentro}, atteso defender@defender ` +
...
tornato === "attacker@attacker"
    ? ok(`uscendo si torna su ${tornato}`)
    : ko(`uscendo il prompt dice ${tornato}, atteso attacker@attacker`);
