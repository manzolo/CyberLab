...
// sparisce, esce dal riquadro. Su un telefono da 390px si leggeva "Cyber Lab"
// per meta' dietro il pulsante "Capitoli", e la sigla si spezzava sul trattino,
// "EDU-" sopra e "CYBER" sotto, dentro il rettangolo azzurro.
//
...
const URL_BASE = process.argv[2] || "http://127.0.0.1:8803/";
const PORTA = 9557;
...
const profilo = mkdtempSync(join(tmpdir(), "cyberlab-testa-"));
...
const MISURA = `(() => {
    ...
    // e' cosi' che si vede "EDU-" sopra e "CYBER" sotto senza guardare l'immagine.
...
for (const lang of ["it", "en"]) {
    ...
    if (!spezzate.length) ok(`${lang}: la sigla EDU-CYBER resta su una riga (${q} larghezze)`);
