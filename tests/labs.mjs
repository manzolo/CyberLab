...
const SEMI = process.env.LAB_SEEDS
    ? process.env.LAB_SEEDS.split(",").map(Number)
    : [424242, 7, 999983];
...
async function caricaScript(cap, es, nome) {
    ...
    const sintassi = childProcess.spawnSync("sh", ["-n", p], { encoding: "utf8" });
    if (sintassi.status !== 0) {
        ko(`${cap}.${es}: ${nome} non e' POSIX valido — ${sintassi.stderr.trim()}`);
        return null;
    }
...
async function provaInfrastruttura() {
    ...
    if (r.ok) ok("topologia isolata, PHP, firewall, filtri e servizi");
    else ko(`infrastruttura NON valida — ${(r.out || "").replace(/\n/g, " | ").slice(0, 700)}`);
}

async function provaIsolamentoRete() {
    await caricaScript("ch01", "e1", "seed.sh");
    await chiedi("seed", "ch01/e1", 424242);
    const rete = await chiedi("sh", "printf '%s/%s/default=%s' \"$(cat /run/lab/pc_ip)\" \"$(cat /run/lab/srv_ip)\" \"$(ip route show default)\"");
    if (rete.out?.trim() === "10.10.0.1/10.10.0.2/default=")
        ok("la LAN e' interna e non ha una route predefinita");
    else
        ko(`isolamento rete inatteso: ${rete.out?.trim()}`);
}
...
async function provaEsercizio(cap, es) {
    ...
    await chiedi("seed", dir, SEMI.at(-1));
