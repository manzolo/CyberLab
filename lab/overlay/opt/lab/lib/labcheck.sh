...
lab_srv_user() { cat "$LAB_RUN/srv_user" 2>/dev/null || echo defender; }
lab_pc_user()  { cat "$LAB_RUN/pc_user"  2>/dev/null || echo attacker; }
...
# rifiuto contiene anche lui il nome ("defender@10.10.0.2: Permission denied"),
...
# ---------------------------------------------------------------- cyber lab
# Tutte le sonde partono dalla pila di rete predefinita (attacker, 10.10.0.1) e
# colpiscono solo 10.10.0.2. La VM non ha una route verso Internet: anche gli
# strumenti generici installati nell'immagine restano chiusi nella LAN del lab.

cyber_def() { /run/lab/entra-server "$@"; }

cyber_fail2ban_stop() {
    # Non serve avviare un secondo interprete Python solo per chiedere al demone
    # di terminare: il pid è il testimone locale e SIGTERM è l'arresto normale.
    if [ -s /run/fail2ban/fail2ban.pid ]; then
        kill "$(cat /run/fail2ban/fail2ban.pid)" 2>/dev/null || true
        i=0
        while [ "$i" -lt 20 ] && kill -0 "$(cat /run/fail2ban/fail2ban.pid)" 2>/dev/null; do
            sleep 0.1; i=$((i + 1))
        done
    fi
...
cyber_firewall_reset() {
    cyber_fail2ban_stop
    # Le catene fail2ban possono essere ancora referenziate: prima si svuotano le
    # catene base, poi si eliminano quelle utente. Tutto nel netns del difensore.
    cyber_def iptables -P INPUT ACCEPT 2>/dev/null || true
    cyber_def iptables -F INPUT 2>/dev/null || true
    cyber_def iptables -F 2>/dev/null || true
    cyber_def iptables -X 2>/dev/null || true
}

cyber_reset() {
    cyber_firewall_reset
    rm -f /etc/cyber-lab/hardened-xss /etc/cyber-lab/hardened-traversal \
          /etc/cyber-lab/hardened-rce
    : > /var/log/cyber-lab/audit.log
    : > /var/lib/cyber-lab/store.txt
    chown defender:defender /var/log/cyber-lab/audit.log /var/lib/cyber-lab/store.txt
    sed -i 's/^enabled = .*/enabled = false/' /etc/fail2ban/jail.d/cyber-lab.conf
    /opt/lab/bin/cyber-services-up >/dev/null 2>&1
    ip addr show dev veth-pc | grep -q '10.10.0.3/24' || \
        ip addr add 10.10.0.3/24 dev veth-pc
}

cyber_http() {
    curl -sS --max-time 4 "$1"
}

cyber_porta_aperta() {
    timeout 4 nc -z -w 2 10.10.0.2 "$1" >/dev/null 2>&1
}

cyber_log_conta() {
    grep -Ec "$1" /var/log/cyber-lab/audit.log 2>/dev/null || true
}

cyber_counter_reject() {
    chain=${1:-INPUT}
    cyber_def iptables -L "$chain" -n -v -x 2>/dev/null \
        | awk '$3 == "REJECT" || $3 == "DROP" { n += $1 } END { print n + 0 }'
}
...
#
# E' il testimone: sshd scrive "Accepted publickey for defender ... ED25519
