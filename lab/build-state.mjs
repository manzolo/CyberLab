...
emulator.add_listener("serial0-output-byte", (b) => {
    ...
    if (fase === "scalda") {
        ...
        // Fail2ban porta con se' l'interprete Python e parecchi moduli. Dal 9p il
        // primo avvio supera abbondantemente il timeout del canale di verifica;
        // nello snapshot, invece, restano calde le pagine lette. Si avviano e si
        // arrestano davvero sia il demone sia fail2ban-regex, poi si ripuliscono
        // configurazione e firewall: il lab consegnato resta nello stato iniziale.
        fase = "scalda-fail2ban";
        testo = "";
        emulator.serial0_send("sudo /opt/lab/bin/cyber-scalda-fail2ban && echo FAIL2BAN-CALDO\n");
        return;
    }
    if (fase === "scalda-fail2ban") {
        if (!/FAIL2BAN-CALDO/.test(testo)) {
            console.error("\nERRORE: non sono riuscito a scaldare Fail2ban");
            console.error(testo.slice(-800));
            process.exit(1);
        }
        clearInterval(punti);
        console.log(`fail2ban scaldato a ${((Date.now() - t0) / 1000).toFixed(1)} s`);
        ...
        // qui e' boot -> crng -> mondo -> scalda -> scalda-fail2ban -> salva.
        fase = "save";
        testo = "";
