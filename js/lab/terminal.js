...
export function creaTerminale(contenitore, uart) {
    ...
    term.onData(d => {
        if (inputAbilitato) accoda(uart, new TextEncoder().encode(normalizzaInputTerminale(d)));
    });
 ...
 *  ferma xterm; la guardia in onData e' la seconda cintura per input gia' in coda
 *  nel browser. */
