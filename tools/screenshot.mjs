...
const profilo = mkdtempSync(join(tmpdir(), "cyberlab-shot-"));
...
async function attendiMacchine() {
    for (let i = 0; i < 120; i++) {
        if (await val("!!(window.__cyberlab && document.getElementById('labStato').classList.contains('pronta'))")) return true;
        await dormi(1000);
    }
...
// ---------------------------------------------------------------- 2. l'handshake
await digita(0, `ssh defender@${ip}\n`);
await dormi(6000);
