...
const PORTA = 9466;
const profilo = mkdtempSync(join(tmpdir(), "cyberlab-btn-"));
...
await val(`localStorage.setItem('cyberlab.introSeen','true')`);
...
const risposta = await val(`window.__cyberlab.agente.ping().then(r => r.out).catch(e => 'ERRORE ' + e.message)`);
