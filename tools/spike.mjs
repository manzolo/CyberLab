...
async function principale() {
    console.log("\n=== prova dell'architettura EDU-CYBER ===");
    const t0 = Date.now();
    for (let t = 1; ; t++) {
    ...
    const mondo = await passo("il mondo dentro lo snapshot", `
        ...
        getent passwd attacker defender | cut -d: -f1,6 | tr '\\n' ' '; echo
        ls /home/defender/.ssh/authorized_keys 2>/dev/null && echo "ATTENZIONE: chiave del riscaldamento rimasta" || echo "pulizia: ok (nessuna chiave gia' autorizzata)"
    `, 120000);
    ...
    const kg = await passo("lo studente genera la chiave", `
        rm -rf /home/attacker/.ssh /home/defender/.ssh
        su attacker -c "mkdir -p ~/.ssh; chmod 700 ~/.ssh; ssh-keygen -t ed25519 -N '' -C attacker@pc -f ~/.ssh/id_ed25519 -q </dev/null"
        ssh-keygen -lf /home/attacker/.ssh/id_ed25519.pub
    `, 180000);

    const copia = await passo("ssh-copy-id porta la chiave sul server (via rete, con la password)", `
        install -d -m 700 -o defender -g defender /home/defender/.ssh
        install -m 600 -o defender -g defender /home/attacker/.ssh/id_ed25519.pub /home/defender/.ssh/authorized_keys
        echo "autorizzata:"; ssh-keygen -lf /home/defender/.ssh/authorized_keys
    `, 180000);

    // --- LA MISURA ---------------------------------------------------------------
    const opz = "-n -o BatchMode=yes -o ConnectTimeout=20";
    const l1 = await passo("PRIMO login della sessione",
        `su attacker -c "ssh ${opz} -o StrictHostKeyChecking=accept-new defender@10.10.0.2 'id -un; hostname'" </dev/null 2>&1 | tail -3`, 250000);
    esito("il PRIMO ssh entra", l1.out.includes("defender"), `${l1.dt.toFixed(1)} s`);

    const l2 = await passo("secondo login", `su attacker -c "ssh ${opz} defender@10.10.0.2 'id -un'" </dev/null 2>&1 | tail -2`, 250000);
    esito("il secondo ssh entra", l2.out.includes("defender"), `${l2.dt.toFixed(1)} s`);

    const l3 = await passo("terzo login", `su attacker -c "ssh ${opz} defender@10.10.0.2 'id -un'" </dev/null 2>&1 | tail -2`, 250000);
    ...
    const senza = await passo("senza la chiave non si entra", `
        mv /home/attacker/.ssh/id_ed25519 /tmp/via
        su attacker -c "ssh ${opz} defender@10.10.0.2 'id -un'" </dev/null 2>&1 | tail -1
        mv /tmp/via /home/attacker/.ssh/id_ed25519
    `, 250000);
    // NB: non si puo' cercare l'assenza della parola "defender" — il messaggio di
    // rifiuto e' "defender@10.10.0.2: Permission denied" e la contiene. L'invariante
